...
function audit_details_display($details)
{
    ...
    // Branch codes / ticket numbers are system data. Keep them raw in every language.
    $d = preg_replace('/\b(电脑|電腦|Computer|computer|Komputer|komputer)-?(\d{6}-\d+)\b/u', 'PC-$2', $d);
    $d = preg_replace('/\bPC(?=\d{6}-\d+)\b/u', 'PC-', $d);

    $lang = function_exists('hd_lang') ? hd_lang() : 'en';

    $statusKeys = [
        'New' => 'New Ticket', 'Open' => 'New Ticket', 'New 工单' => 'New Ticket', '新工单' => 'New Ticket', 'New Ticket' => 'New Ticket',
        '处理中' => 'In Progress', 'In Progress' => 'In Progress',
        'Pending' => 'Waiting Reply', '等待回复' => 'Waiting Reply', 'Waiting Reply' => 'Waiting Reply',
        '已解决' => 'Solved', 'Solved' => 'Solved',
        '已关闭' => 'Closed', 'Closed' => 'Closed',
        '已取消' => 'Cancelled', 'Cancelled' => 'Cancelled', 'Canceled' => 'Cancelled'
    ];
    $statusText = [
        'en' => ['New Ticket'=>'New Ticket','In Progress'=>'In Progress','Waiting Reply'=>'Waiting Reply','Solved'=>'Solved','Closed'=>'Closed','Cancelled'=>'Cancelled'],
        'ms' => ['New Ticket'=>'Tiket Baharu','In Progress'=>'Dalam Proses','Waiting Reply'=>'Menunggu Balasan','Solved'=>'Selesai','Closed'=>'Ditutup','Cancelled'=>'Dibatalkan'],
        'zh' => ['New Ticket'=>'新工单','In Progress'=>'处理中','Waiting Reply'=>'等待回复','Solved'=>'已解决','Closed'=>'已关闭','Cancelled'=>'已取消']
    ];
    $roleText = [
        'en' => ['admin'=>'admin','head'=>'head','staff'=>'staff'],
        'ms' => ['admin'=>'admin','head'=>'ketua','staff'=>'staf'],
        'zh' => ['admin'=>'管理员','head'=>'主管','staff'=>'员工']
    ];

    $trStatus = function($value) use ($statusKeys, $statusText, $lang) {
        $value = trim((string)$value);
        $key = $statusKeys[$value] ?? $value;
        return $statusText[$lang][$key] ?? $value;
    };
    $trRole = function($value) use ($roleText, $lang) {
        $value = strtolower(trim((string)$value));
        return $roleText[$lang][$value] ?? $value;
    };

    // Normalize and translate Quick Action lines:
    // Ticket BS-202606-001 status: New Ticket -> New Ticket, assigned: Unassigned -> TEAM LEADER
    if(preg_match('/^(Ticket|工单|Tiket)\s+([^\s]+)\s+status:\s*(.*?)\s*->\s*(.*?)(?:,\s*assigned:\s*(.*?)\s*->\s*(.*))?$/u', $d, $m)){
        $ticketNo = audit_display_raw_code($m[2] ?? '');
        $oldStatus = $trStatus($m[3] ?? '');
        $newStatus = $trStatus($m[4] ?? '');
        $oldAssignee = audit_display_raw_code($m[5] ?? '');
        $newAssignee = audit_display_raw_code($m[6] ?? '');

        if($lang === 'zh'){
            $out = '工单 '.$ticketNo.' 状态：'.$oldStatus.' → '.$newStatus;
            if(isset($m[5]) && trim($m[5]) !== '') $out .= '，指派给：'.$oldAssignee.' → '.$newAssignee;
            return $out;
        }
        if($lang === 'ms'){
            $out = 'Tiket '.$ticketNo.' status: '.$oldStatus.' → '.$newStatus;
            if(isset($m[5]) && trim($m[5]) !== '') $out .= ', ditugaskan kepada: '.$oldAssignee.' → '.$newAssignee;
            return $out;
        }
        $out = 'Ticket '.$ticketNo.' status: '.$oldStatus.' → '.$newStatus;
        if(isset($m[5]) && trim($m[5]) !== '') $out .= ', assigned: '.$oldAssignee.' → '.$newAssignee;
        return $out;
    }

    // Normalize and translate user role edit lines:
    // user ID 5 role staff
    if(preg_match('/^(?:Updated\s+)?user\s+ID\s+(\d+)\s+role\s+(admin|head|staff)$/iu', trim($d), $m)){
        if($lang === 'zh') return '已更新用户 ID '.$m[1].' 角色 '.$trRole($m[2]);
        if($lang === 'ms') return 'Pengguna ID '.$m[1].' peranan '.$trRole($m[2]).' telah dikemas kini';
        return 'Updated user ID '.$m[1].' role '.$trRole($m[2]);
    }

    // Normalize and translate logout lines.
    if(preg_match('/^(User\s+)?logged\s+out$/iu', trim($d))){
        if($lang === 'zh') return '用户已退出登录';
        if($lang === 'ms') return 'Pengguna telah log keluar';
        return 'User logged out';
    }

    // Normalize and translate toggle ticket status lines.
    if(preg_match('/^toggle\s+ticket\s+status$/iu', trim($d))){
        if($lang === 'zh') return '切换工单状态';
        if($lang === 'ms') return 'Tukar Status Tiket';
        return 'Toggle Ticket Status';
    }

    // Normalize and translate ticket status management lines.
    if(preg_match('/^(delete|add|edit)\s+ticket\s+status\s*(.*)$/iu', trim($d), $m)){
        $verb = strtolower($m[1]);
        $tail = trim((string)($m[2] ?? ''));
        $tail = $tail !== '' ? $trStatus($tail) : '';
        if($lang === 'zh'){
            $verbZh = ['delete'=>'删除','add'=>'添加','edit'=>'编辑'][$verb] ?? $verb;
            return trim($verbZh.'工单状态'.($tail !== '' ? ' '.$tail : ''));
        }
        if($lang === 'ms'){
            $verbMs = ['delete'=>'Padam','add'=>'Tambah','edit'=>'Edit'][$verb] ?? $verb;
            return trim($verbMs.' status tiket'.($tail !== '' ? ' '.$tail : ''));
        }
        return trim(ucfirst($verb).' ticket status'.($tail !== '' ? ' '.$tail : ''));
    }

    // Normalize and translate role permission matrix line.
    if(preg_match('/Head\/Staff\s+role\s+permission\s+checkbox\s+matrix\.?/iu', $d)){
        if($lang === 'zh') return '更新 Head/Staff 角色权限复选框矩阵。';
        if($lang === 'ms') return 'Kemas kini matriks kotak pilihan kebenaran peranan Head/Staff.';
        return 'Updated Head/Staff role permission checkbox matrix.';
    }

    // Normalize and translate simpler status update lines:
    // Ticket BS-202606-009 changed to In Progress
    if(preg_match('/^(Ticket|工单|Tiket)\s+([^\s]+)\s+changed\s+to\s+(.+)$/iu', trim($d), $m)){
        $ticketNo = audit_display_raw_code($m[2] ?? '');
        $newStatus = $trStatus($m[3] ?? '');
        if($lang === 'zh') return '工单 '.$ticketNo.' 状态已更改为 '.$newStatus;
        if($lang === 'ms') return 'Tiket '.$ticketNo.' ditukar kepada '.$newStatus;
        return 'Ticket '.$ticketNo.' changed to '.$newStatus;
    }

    $replaceStatusWords = function($text) use ($statusKeys, $statusText, $lang) {
        $items = array_keys($statusKeys);
        usort($items, function($a, $b){ return mb_strlen($b, 'UTF-8') <=> mb_strlen($a, 'UTF-8'); });
        foreach($items as $src){
            $key = $statusKeys[$src] ?? $src;
            $dst = $statusText[$lang][$key] ?? $src;
            $text = preg_replace('/(?<![A-Za-z])'.preg_quote($src, '/').'(?![A-Za-z])/u', $dst, $text);
        }
        return $text;
    };
    ...
    if($lang === 'zh'){
        $repl = [
            'status:' => '状态：', 'assigned:' => '指派给：', 'changed to' => '状态已更改为',
            ...
            'Edited announcement'=>'编辑公告', 'Edited Announcement'=>'编辑公告',
            'Created announcement'=>'创建公告', 'Created Announcement'=>'创建公告',
            'announcement'=>'公告',
        ...
        $d = strtr($d, $repl);
        $d = $replaceStatusWords($d);
        return $d;
    }

    if($lang === 'ms'){
        $repl = [
            'status:' => 'status:', 'assigned:' => 'ditugaskan kepada:', 'changed to' => 'ditukar kepada',
            ...
            'Edited announcement'=>'Edit pengumuman', 'Edited Announcement'=>'Edit pengumuman',
            'Created announcement'=>'Cipta pengumuman', 'Created Announcement'=>'Cipta pengumuman',
    ...
    $repl = [
        '编辑公告'=>'Edited announcement', '创建公告'=>'Created announcement', '公告'=>'announcement',
        '状态：'=>'status:', '指派给：'=>'assigned:', '状态已更改为'=>'changed to',
        '删除资产 '=>'Deleted Asset ', '删除工单 '=>'Deleted Ticket ', 'Deleted 工单 '=>'Deleted Ticket ', '创建时间 工单 '=>'Created ticket ',
        '创建工单 '=>'Created ticket ', '回复工单 '=>'Ticket ', '已回复'=>' replied', '用户已退出登录'=>'User logged out'
    ];
    $d = strtr($d, $repl);
    $d = $replaceStatusWords($d);
    return $d;
...
<div class="card table-card">
    <div class="card-body p-0">
        <div class="d-flex justify-content-between align-items-center p-3 border-bottom">
            <div><strong><?= htmlspecialchars(__('Activity List')); ?></strong><div class="text-muted small"><?= htmlspecialchars(__('Showing')); ?> <?= number_format(count($logs)); ?> <?= htmlspecialchars(__('of')); ?> <?= number_format($total_rows); ?> <?= htmlspecialchars(__('filtered records')); ?></div></div>
            <div class="text-muted small no-print"><?= htmlspecialchars(__('Page')); ?> <?= $page; ?> / <?= $total_pages; ?></div>
        </div>
        ...
        <div class="d-flex flex-wrap justify-content-between align-items-center p-3 border-top no-print">
            <div class="text-muted small"><?= htmlspecialchars(__('Showing')); ?> <?= $total_rows ? number_format($offset + 1) : 0; ?> <?= htmlspecialchars(__('to')); ?> <?= number_format(min($offset + $per_page, $total_rows)); ?> <?= htmlspecialchars(__('of')); ?> <?= number_format($total_rows); ?> <?= htmlspecialchars(__('records')); ?></div>
            <div class="btn-group">
                <a class="btn btn-outline-secondary <?= $page <= 1 ? 'disabled' : ''; ?>" href="audit_logs.php?<?= htmlspecialchars(build_query_keep(['page'=>$page-1])); ?>">‹ <?= htmlspecialchars(__('Previous')); ?></a>
                <a class="btn btn-soft disabled" href="#"><?= $page; ?></a>
                <a class="btn btn-outline-secondary <?= $page >= $total_pages ? 'disabled' : ''; ?>" href="audit_logs.php?<?= htmlspecialchars(build_query_keep(['page'=>$page+1])); ?>"><?= htmlspecialchars(__('Next')); ?> ›</a>
            </div>
        </div>
    </div>
</div>
