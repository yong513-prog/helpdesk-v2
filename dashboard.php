...
function dash_current_lang(): string
{
    $lang = (string)($_GET['lang'] ?? $_SESSION['lang'] ?? $_COOKIE['lang'] ?? $_SESSION['language'] ?? 'en');
    $lang = strtolower(trim($lang));
    if(in_array($lang, ['zh','cn','zh-cn','zh_cn','chinese'], true)) return 'zh';
    if(in_array($lang, ['ms','my','bm','malay','bahasa','bahasa_melayu'], true)) return 'ms';
    return 'en';
}

function dash_t(string $key): string
{
    static $dict = [
        'en' => [
            'welcome_back' => 'Welcome back',
            'dashboard_summary' => 'Here is what is happening with your helpdesk today.',
            'malaysia_time' => 'Malaysia Time',
            'create_ticket' => 'Create Ticket',
            'read_only' => 'Read Only',
            'no_create_permission' => 'No create permission',
            'active_announcement' => 'Active Announcement',
            'no_company_notice' => 'There are no company notices at the moment.',
            'new' => 'New',
        ],
        'zh' => [
            'welcome_back' => '欢迎回来',
            'dashboard_summary' => '这里是今天 Helpdesk 的摘要。',
            'malaysia_time' => '马来西亚时间',
            'create_ticket' => '创建工单',
            'read_only' => '只读',
            'no_create_permission' => '没有创建权限',
            'active_announcement' => '当前公告',
            'no_company_notice' => '目前没有公司公告。',
            'new' => '新',
        ],
        'ms' => [
            'welcome_back' => 'Selamat kembali',
            'dashboard_summary' => 'Ini ringkasan Helpdesk anda hari ini.',
            'malaysia_time' => 'Masa Malaysia',
            'create_ticket' => 'Cipta Tiket',
            'read_only' => 'Baca Sahaja',
            'no_create_permission' => 'Tiada kebenaran cipta',
            'active_announcement' => 'Pengumuman Aktif',
            'no_company_notice' => 'Tiada pengumuman syarikat buat masa ini.',
            'new' => 'Baharu',
        ],
    ];
    $lang = dash_current_lang();
    return $dict[$lang][$key] ?? $dict['en'][$key] ?? $key;
}

$dashLang = dash_current_lang();
...
<div class="dash-wrap">
    <div class="top-actions">
        <div class="welcome-card">
            ...
            <div><h5 class="welcome-title"><?= h(dash_t('welcome_back')); ?>, <?= h($_SESSION['username'] ?? 'User'); ?>! 👋</h5><div class="text-muted small"><?= h(dash_t('dashboard_summary')); ?></div></div>
        </div>
        ...
        <div class="mini-card"><div class="mini-icon"><i class="bi bi-clock"></i></div><div><strong id="liveTime"></strong><br><small class="text-muted"><?= h(dash_t('malaysia_time')); ?></small></div></div>
        <?php if(current_user_has_permission('create_ticket')): ?><a href="create_ticket.php" class="btn btn-primary create-btn"><i class="bi bi-plus-lg me-2"></i><?= h(dash_t('create_ticket')); ?></a><?php else: ?><div class="mini-card"><div class="mini-icon"><i class="bi bi-shield-lock"></i></div><div><strong><?= h(dash_t('read_only')); ?></strong><br><small class="text-muted"><?= h(dash_t('no_create_permission')); ?></small></div></div><?php endif; ?>
    </div>
    ...
    <div class="announcement-strip p-3 mb-3">
        ...
        <div class="ann-grid">
            ...
            <a href="announcements.php" class="ann-item text-decoration-none">
                ...
                <div><div class="ann-name"><?= h(hd_announcement_title($pdo, $a)); ?><span class="small-badge-new"><?= h(dash_t('new')); ?></span></div><div class="ann-meta"><?= h($a['created_at'] ?? ''); ?></div><div class="ann-content"><?= h(mb_substr(strip_tags(hd_announcement_content($pdo, $a)),0,110)); ?><?= mb_strlen(strip_tags(hd_announcement_content($pdo, $a)))>110?'...':''; ?></div></div>
            </a>
            <?php endforeach; else: ?>
            <div class="ann-item"><div class="ann-icon"><i class="bi bi-megaphone"></i></div><div><div class="ann-name"><?= h(dash_t('active_announcement')); ?></div><div class="ann-content"><?= h(dash_t('no_company_notice')); ?></div></div></div>
            <?php endif; ?>
        </div>
    </div>
...
<script>
const dashLang = <?= json_encode($dashLang); ?>;
...
function updateDateTime(){
    ...
    let dateText, dayText, timeText;
    if(dashLang === 'zh'){
        dateText = y + '年' + (m+1) + '月' + d + '日';
        dayText = weekdays.zh[now.getDay()];
        const h24 = now.getHours();
        const mer = h24 < 12 ? '上午' : '下午';
        const h12 = h24 % 12 || 12;
        timeText = pad2(h12) + ':' + pad2(now.getMinutes()) + ':' + pad2(now.getSeconds()) + ' ' + mer;
    } else if(dashLang === 'ms'){
        dateText = pad2(d) + ' ' + months.ms[m] + ' ' + y;
        dayText = weekdays.ms[now.getDay()];
        const h24 = now.getHours();
        const mer = h24 < 12 ? 'PG' : 'PTG';
        const h12 = h24 % 12 || 12;
        timeText = pad2(h12) + ':' + pad2(now.getMinutes()) + ':' + pad2(now.getSeconds()) + ' ' + mer;
    } else {
        dateText = pad2(d) + ' ' + months.en[m] + ' ' + y;
        dayText = weekdays.en[now.getDay()];
        const h24 = now.getHours();
        const mer = h24 < 12 ? 'AM' : 'PM';
        const h12 = h24 % 12 || 12;
        timeText = pad2(h12) + ':' + pad2(now.getMinutes()) + ':' + pad2(now.getSeconds()) + ' ' + mer;
    }
    document.getElementById('liveDate').textContent = dateText;
    document.getElementById('liveDay').textContent = dayText;
    document.getElementById('liveTime').textContent = timeText;
}
