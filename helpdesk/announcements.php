...
if($show === 'active') {
    $where .= " AND (a.start_date IS NULL OR a.start_date <= CURDATE()) AND (a.end_date IS NULL OR a.end_date >= CURDATE()) ";
}
...
$totalActive = (int)$pdo->query("SELECT COUNT(*) FROM announcements a WHERE (a.start_date IS NULL OR a.start_date <= CURDATE()) AND (a.end_date IS NULL OR a.end_date >= CURDATE())")->fetchColumn();
...
$totalReadRows = 0;
if($totalActive > 0) {
    $totalReadRows = (int)$pdo->query("SELECT COUNT(*) FROM announcement_reads ar INNER JOIN announcements a ON a.id = ar.announcement_id WHERE (a.start_date IS NULL OR a.start_date <= CURDATE()) AND (a.end_date IS NULL OR a.end_date >= CURDATE())")->fetchColumn();
}
...
<form method="get" class="ann-toolbar">
    <div class="row g-3 align-items-center">
        ...
        <div class="col-lg-2"><select name="show" class="form-select form-select-lg"><option value="active" <?= $show==='active'?'selected':''; ?>><?= __('Active Only') ?></option><option value="all" <?= $show==='all'?'selected':''; ?>><?= __('All') ?></option></select></div>
...
<div class="row g-3 mb-4">
    <div class="col-md-6 col-xl-3"><div class="ann-stat"><div class="ann-stat-icon purple"><i class="bi bi-megaphone-fill"></i></div><div><div class="ann-stat-number"><?= $totalActive; ?></div><div class="ann-stat-label"><?= __('Total Announcements') ?></div><div class="ann-stat-note"><?= __('Active announcements') ?></div></div></div></div>
...
<div class="ann-table-card">
    ...
    <?php foreach($announcements as $row):
        ...
        $isActive = ((empty($row['start_date']) || $row['start_date'] <= date('Y-m-d')) && (empty($row['end_date']) || $row['end_date'] >= date('Y-m-d')));
    ...
    <div class="ann-row">
        <div>
            ...
            <div class="ann-meta"><span class="ann-badge <?= $isActive?'active':'inactive'; ?>"><?= __($isActive?'Active':'Inactive'); ?></span> &nbsp; <?= __('Posted by') ?> <b><?= htmlspecialchars($row['created_by_name']); ?></b> &nbsp;·&nbsp; <?= htmlspecialchars($row['created_at']); ?></div>
        ...
        <div><span class="ann-badge <?= $isActive?'active':'inactive'; ?>"><?= __($isActive?'Active':'Inactive'); ?></span></div>
