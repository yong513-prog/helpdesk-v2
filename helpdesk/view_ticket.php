...
<div class="mobile-detail-toggle-row">
    ...
    <div class="ticket-stat-grid" id="ticketStatGrid">
        ...
        <div class="ticket-stat"><div class="stat-icon" style="background:#7c3aed;"><i class="bi bi-person-badge"></i></div><div><div class="stat-label"><?= h(__('Person In Charge')) ?></div><div class="stat-value"><?= h($ticket['department'] ?: '-'); ?></div></div></div>
                    ...
                    <div class="timeline">
                        <?php foreach($timelineLogs as $log): ?>
                            <div class="timeline-item">
                                ...
                                <div>
                                    <div class="timeline-title"><?= h(__(preg_replace('/\s*\(\d+\)\s*$/', '', (string)$log['action'])) . (preg_match('/\s*(\(\d+\))\s*$/', (string)$log['action'], $m) ? ' '.$m[1] : '')); ?></div>
