...
for($d=1; $d <= $daysInMonth; $d++){
    ...
    $dailyLabels[] = date('M d', strtotime($dayKey));
...
<div class="report-wrap">
    ...
    <div class="kpi-grid">
        <div class="kpi-box">
            <div class="kpi-label"><?= __('Total Tickets') ?></div><div class="kpi-value"><?= (int)$totalTickets; ?></div><div class="kpi-note">100% <?= __('of all tickets') ?></div><div class="kpi-icon kpi-blue"><i class="bi bi-clipboard-data"></i></div>
        </div>
        <div class="kpi-box">
            <div class="kpi-label"><?= __('Active Tickets') ?></div><div class="kpi-value"><?= (int)$activeTickets; ?></div><div class="kpi-note"><?= h($activePercent); ?>% <?= __('of total') ?></div><div class="kpi-icon kpi-green"><i class="bi bi-activity"></i></div>
        </div>
        <div class="kpi-box">
            <div class="kpi-label"><?= __('Closed Tickets') ?></div><div class="kpi-value"><?= (int)$closedTickets; ?></div><div class="kpi-note"><?= h($closedPercent); ?>% <?= __('of total') ?></div><div class="kpi-icon kpi-purple"><i class="bi bi-check2-circle"></i></div>
        </div>
        <div class="kpi-box">
            <div class="kpi-label"><?= __('Overdue Tickets') ?></div><div class="kpi-value text-danger"><?= (int)$overdueTickets; ?></div><div class="kpi-note"><?= h($overduePercent); ?>% <?= __('of total') ?></div><div class="kpi-icon kpi-red"><i class="bi bi-alarm"></i></div>
        </div>
        <div class="kpi-box">
    ...
    <div class="status-layout">
                ...
                <table class="table table-hover mb-0">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td><div class="progress-mini <?= ((int)$row['is_closed'] || (int)$row['count'] === 0) ? '' : 'danger' ?>"><span style="width:<?= h(min(100, (float)$row['percent'])); ?>%"></span></div></td>
                        </tr>
                        <?php endforeach; ?>
        ...
        <div class="report-card">
            <div class="report-card-head"><span><i class="bi bi-pie-chart text-primary me-2"></i><?= __('Tickets by Status') ?></span></div>
    ...
    <div class="summary-grid">
        <div class="report-card">
            ...
            <div class="table-responsive">
                <table class="table table-hover mb-0">
                    <thead><tr><th><?= __('Branch') ?></th><th><?= __('Total') ?></th><th><?= __('Closed') ?></th><th><?= __('Overdue') ?></th><th>% <?= __('Overdue') ?></th></tr></thead>
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td><div class="d-flex align-items-center gap-2"><span class="fw-bold"><?= h($brOverPct); ?>%</span><div class="progress-mini danger"><span style="width:<?= h(min(100,$brOverPct)); ?>%"></span></div></div></td>
                        </tr>
                        <?php endforeach; ?>
                        <?php if(!$branchRows): ?><tr><td colspan="5" class="text-center text-muted py-4"><?= __('No data') ?></td></tr><?php endif; ?>
                    </tbody>
                </table>
            </div>
        </div>
        ...
        <div class="report-card">
            ...
            <div class="table-responsive">
                <table class="table table-hover mb-0">
                    <thead><tr><th><?= __('KPI Person In Charge') ?></th><th><?= __('Total') ?></th><th><?= __('Closed') ?></th><th><?= __('Overdue') ?></th><th>% <?= __('Overdue') ?></th></tr></thead>
                    <tbody>
                        <?php foreach($picRows as $r): $picTotal=max(1,(int)$r['total']); $picOverPct=round(((int)$r['overdue_total']/$picTotal)*100,1); ?>
                        <tr><td class="fw-bold"><?= h($r['department'] ?: '-'); ?></td><td><?= (int)$r['total']; ?></td><td><?= (int)$r['closed_total']; ?></td><td><?= (int)$r['overdue_total']; ?></td><td><div class="d-flex align-items-center gap-2"><span class="fw-bold"><?= h($picOverPct); ?>%</span><div class="progress-mini danger"><span style="width:<?= h(min(100,$picOverPct)); ?>%"></span></div></div></td></tr>
                        <?php endforeach; ?>
                        <?php if(!$picRows): ?><tr><td colspan="5" class="text-center text-muted py-4"><?= __('No data') ?></td></tr><?php endif; ?>
                    </tbody>
                </table>
            </div>
        </div>
        ...
        <div class="report-card">
            ...
            <div class="report-card-body insight-list">
                <div class="insight-item"><div class="insight-icon" style="background:#3b82f6"><i class="bi bi-ticket-detailed"></i></div><div><div class="insight-title"><?= h($activePercent); ?>% <?= __('tickets are active') ?></div><div class="insight-text"><?= __('Need triage and assignment') ?></div></div></div>
                <div class="insight-item"><div class="insight-icon" style="background:#22c55e"><i class="bi bi-shield-check"></i></div><div><div class="insight-title"><?= __('SLA Compliance') ?> <?= $slaPercent >= 95 ? __('is Excellent') : __('Needs Attention') ?></div><div class="insight-text"><?= __('Target') ?> ≥ 95%</div></div></div>
                <div class="insight-item"><div class="insight-icon" style="background:#f59e0b"><i class="bi bi-alarm"></i></div><div><div class="insight-title"><?= (int)$overdueTickets; ?> <?= __('ticket(s) are overdue') ?></div><div class="insight-text"><?= __('Requires immediate attention') ?></div></div></div>
    ...
    <div class="report-card trend-card">
        <div class="report-card-head"><span><i class="bi bi-graph-up text-primary me-2"></i><?= __('Ticket Trend') ?> (<?= __('Daily') ?>)</span><span class="text-muted small"><?= h(date('F Y', strtotime($startDate))); ?></span></div>
