...
require_module_permission('sla_management');
...
if($_SERVER['REQUEST_METHOD'] === 'POST')
{
    ...
    $sla_hours = trim($_POST['sla_hours'] ?? '');
    $id = (int)($_POST['id'] ?? 0);
    $status = (int)($_POST['status'] ?? 1);

    if($action === 'add')
    {
        if($priority_name === '' || $sla_hours === '')
        {
        ...
        else
        {
            ...
            {
                $stmt = $pdo->prepare("
                    INSERT INTO sla_master
                    (priority_name, sla_hours, status)
                    VALUES (?, ?, ?)
                ");
                $stmt->execute([
                $priority_name,
                ...
                $message = 'SLA Management added successfully.';
            }
            catch(Exception $e)
            {
    ...
    if($action === 'edit')
    {
        if($id <= 0 || $priority_name === '' || $sla_hours === '')
        {
            $error = 'Invalid data.';
        }
        else
        {
            try
            {
                ...
                $message = 'SLA Management updated successfully.';
            }
            catch(Exception $e)
            {
    ...
    if($action === 'delete')
    {
        if($id > 0)
        {
            $stmt = $pdo->prepare("SELECT priority_name FROM sla_master WHERE id = ?");
            $stmt->execute([$id]);
            $deleteName = trim((string)$stmt->fetchColumn());

            if($deleteName === '')
            {
                $error = 'SLA Priority not found.';
            }
            else
            {
                $used = 0;
                try { $s = $pdo->prepare("SELECT COUNT(*) FROM tickets WHERE priority = ?"); $s->execute([$deleteName]); $used += (int)$s->fetchColumn(); } catch(Exception $e) {}
                try { $s = $pdo->prepare("SELECT COUNT(*) FROM ticket_category_master WHERE default_priority = ?"); $s->execute([$deleteName]); $used += (int)$s->fetchColumn(); } catch(Exception $e) {}

                if($used > 0)
                {
                    $stmt = $pdo->prepare("UPDATE sla_master SET status = 0, updated_at = NOW() WHERE id = ?");
                    $stmt->execute([$id]);
                    $message = 'SLA Priority is already used by Tickets / Categories, so it has been disabled instead of deleted.';
                }
                else
                {
                    $stmt = $pdo->prepare("DELETE FROM sla_master WHERE id = ?");
                    $stmt->execute([$id]);
                    $message = 'Unused SLA Priority deleted successfully.';
                }
            }
        }
    }
...
@media(max-width:768px){
    ...
    .master-table td:nth-child(2)::before{content:'Priority Name';}
    .master-table td:nth-child(3)::before{content:'SLA Hours';}
    .master-table td:nth-child(4)::before{content:'Status';}
...
<div class="master-page-header">
    <div>
        ...
        <div class="text-muted">Add, edit, disable or delete SLA priority rules.</div>
    </div>
...
<div class="master-card">
    <div class="master-card-header">
        <div>
            <strong>Add SLA</strong>
            <div class="text-muted small">Add, edit, disable or delete SLA priority rules.</div>
        </div>
    </div>
    <div class="master-card-body">
        <form method="post" class="row g-3">
            ...
            <div class="col-md">
                ...
                <input type="text" name="priority_name" class="form-control" value="" required>
            </div>
            <div class="col-md">
                <label class="form-label">SLA Hours</label>
                <input type="number" name="sla_hours" class="form-control" value="24" required>
            </div>
...
<th>Action</th></tr></thead>
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        <input type="text" name="priority_name" class="form-control form-control-sm" value="<?= esc($item['priority_name'] ?? ''); ?>" required>
                    </td>
                    <td>
                        <input type="number" name="sla_hours" class="form-control form-control-sm" value="<?= esc($item['sla_hours'] ?? ''); ?>" required>
                    </td>
                    <td>
